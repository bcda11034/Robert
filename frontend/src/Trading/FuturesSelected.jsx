...
function FuturesSelected({ futuresAssetType, futuresPriceData }) {
    ...
    return (
        <div>
            ...
            <span class="money-value"> ${currentPrice?.price != null ? Intl.NumberFormat("en-US").format(currentPrice.price) : ""}</span>
            &nbsp;&nbsp;
            <span class={currentPrice?.percent >= 0 ? "percent-plus" : "percent-minus" }>${currentPrice?.percent != null ? (currentPrice.percent * 100).toFixed(2) : ""}%</span>
        </div>
    )
}
